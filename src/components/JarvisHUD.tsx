...
export function JarvisHUD({state = 'idle', analyser, amplitude, response = '', activity = defaultActivity, onCommand, onListen, onStopListening, onStateChange, microphoneActive = false, microphoneStarting = false, audioError, soundEnabled = false, onSoundChange, reducedMotion = false, className = ''}: JarvisHUDProps) {
  ...
  return <div className={`jarvis-hud ${reduced ? 'reduce-motion' : ''} ${className}`} data-state={state}>
    ...
    <div className="interface-shell">
      <header className="topbar">
        ...
        <div className="topbar-right"><span className="connection-badge"><i/> SYSTEM ONLINE</span><span className="topbar-divider"/><button className="icon-button" aria-label={soundEnabled ? 'Mute interface sounds' : 'Enable interface sounds'} aria-pressed={soundEnabled} onClick={() => onSoundChange?.(!soundEnabled)} disabled={!onSoundChange}>{soundEnabled ? <Volume2 size={18}/> : <VolumeX size={18}/>}</button><button className="icon-button fullscreen-button" aria-label="Toggle full screen" onClick={toggleFullscreen}><Maximize2 size={17}/></button><button ref={settingsRef} className={`icon-button ${settingsOpen ? 'selected' : ''}`} aria-label="Interface settings" aria-expanded={settingsOpen} onClick={() => setSettingsOpen(!settingsOpen)}><Settings2 size={18}/></button></div>
      </header>
      <main id={mainId}>
        <section className="welcome-row"><div><div className="eyebrow"><span className="tiny-line"/> PERSONAL INTELLIGENCE INTERFACE <span className="version">V.4.0</span></div><h1>At your service.</h1><p>All systems operational. Ready when you are.</p></div><Clock/></section>
        <div className="workspace-grid">
          <aside aria-hidden={!telemetry} className={`left-instruments ${!telemetry ? 'telemetry-hidden' : ''}`}>
            <Panel title="SYSTEM DIAGNOSTICS" code="01" reduced={reduced} delay={.1}>
          ...
          <section className="core-stage" aria-label="Assistant neural core">
            <div className="stage-heading"><span><span className="crosshair">⌖</span> NEURAL CORE</span><span>ARC REACTOR · MK VII</span></div>
            <div className="core-viewport"><i className="corner corner-tl"/><i className="corner corner-tr"/><i className="corner corner-bl"/><i className="corner corner-br"/><span className="coordinate coordinate-left">SYS. 07 / 084</span><span className="coordinate coordinate-right">NEURAL LINK ESTABLISHED</span>
              ...
              <div className="core-floor"/><div className="orb-caption"><span>CORE SYNC</span><strong>100<span>%</span></strong><span className="caption-line"/><span>LATENCY</span><strong>12<span>ms</span></strong></div>
            </div>
          ...
          <aside className="right-instruments">
            <Panel title="ACTIVE PROTOCOLS" code="03" reduced={reduced} delay={.15}>
              <div className="protocol"><span className="protocol-icon"><AudioLines size={18}/></span><div><strong>Voice recognition</strong><span>{microphoneActive ? 'LIVE MICROPHONE' : 'AWAITING INPUT'}</span></div><i className={microphoneActive ? 'live' : ''}/></div>
              <div className="protocol"><span className="protocol-icon"><ShieldCheck size={18}/></span><div><strong>Security protocol</strong><span>LOCAL SESSION ONLY</span></div><i className="live"/></div>
              <div className="protocol"><span className="protocol-icon"><Globe2 size={18}/></span><div><strong>Network uplink</strong><span>CONNECTION STABLE</span></div><i className="live"/></div>
              <div className="protocol-note"><span className="signal-bars"><i/><i/><i/><i/></span><span>LOCAL INTERFACE</span><span>24 ms</span></div>
            </Panel>
